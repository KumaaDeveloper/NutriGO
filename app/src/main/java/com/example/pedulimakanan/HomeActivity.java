...
public class HomeActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Membaca session login terpusat dari main branch
        ...
        // RecyclerView Setup Grid 2 Kolom
        ...
        // Ambil data restoran dan saldo langsung dari server API remote
    ...
    protected void onResume() {
        super.onResume();
        // Refresh data saldo dan toko secara berkala saat activity aktif kembali
        new LoadHomeDataTask().execute();
    }
    ...
    private void applyFilter() {
        ...
        for (RestoranModel r : allStores) {
            boolean matchCategory = activeCategory.equals("Semua") || r.tipeMenu.equalsIgnoreCase(activeCategory);
    ...
    // ── Setup Top-up Bottom Sheet Sheet Manual ────────────────────────
    ...
    // ── Logika Sinkronisasi Navigasi 5 Menu Utama ────────────────────
    ...
    // ── ASYNCTASK 1: Mengambil Data Restoran & Saldo dari Server ──
    private class LoadHomeDataTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... voids) {
            ...
            try {
                ...
                String postData = URLEncoder.encode("action", "UTF-8") + "=" + URLEncoder.encode("get_home_data", "UTF-8") + "&" +
    ...
    // ── ASYNCTASK 2: Mengirim Request Top Up Saldo ke Server ──
    private class TopupTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                String postData = URLEncoder.encode("action", "UTF-8") + "=" + URLEncoder.encode("topup_saldo", "UTF-8") + "&" +
                        URLEncoder.encode("user_id", "UTF-8") + "=" + URLEncoder.encode(String.valueOf(userId), "UTF-8") + "&" +
                        URLEncoder.encode("amount", "UTF-8") + "=" + URLEncoder.encode(String.valueOf(addedAmount), "UTF-8");
        ...
        protected void onPostExecute(String response) {
            ...
            try {
                JSONObject jsonObject = new JSONObject(response);
                if (jsonObject.optBoolean("success", false)) {
                    Toast.makeText(HomeActivity.this, "Top Up Rp " + fmt(addedAmount) + " berhasil!", Toast.LENGTH_SHORT).show();
                    hideTopupSheet();
                    // Segera refresh data saldo terbaru dari server
                    new LoadHomeDataTask().execute();
                } else {
                    Toast.makeText(HomeActivity.this, jsonObject.optString("message", "Top up gagal"), Toast.LENGTH_SHORT).show();
                }
            } catch (Exception e) {
